...
portStruct diPorts[4] = {
                            {0b00000001, "DI_00", DI_00, 0, false, false},
                            {0b00000010, "DI_01", DI_01, 1, false, false},
                            {0b00000100, "DI_02", DI_02, 2, false, false},
                            {0b00001000, "DI_03", DI_03, 3, false, false}
                        };

portStruct doPorts[4] = {
                            {0b00010000, "DO_00", DO_00, 0, false, false},
                            {0b00100000, "DO_01", DO_01, 1, false, false},
                            {0b01000000, "DO_02", DO_02, 2, false, false},
                            {0b10000000, "DO_03", DO_03, 3, false, false}
                        };
...
void loop()
{
    switch(curState) {
        ...
        case CLIENT_CONNECTED:
            ...
            tftDisplay( "is Connected" , 2, Yellow ,0,80 ,false,Black,0);
            tftDisplay( "to MQTT Broker!!", 2, Yellow ,0,120 ,false,Black,0);
            tftDisplay( "Waiting for Gateway", 2, Yellow ,0,160 ,false,Black,500);
        ...
        case START_PRODUCTION:
            if( prevState == CLIENT_CONNECTED ) {
                ...
                for(int i=0;i<4;i++){
                    tftDisplayNoCenter(diPorts[i].portName, 2, Yellow, 10, (40*i), false,Black,0);
                    tftDisplayNoCenter(":", 2, Yellow, 80, (40*i), false,Black,0);
                    //tftDisplayNoCenterWithNumber(String(val), 2, Yellow, 80, (40*i), false,Black,0);
                    tftDisplayNoCenter(doPorts[i].portName, 2, Yellow, 170, (40*i), false,Black,0);
                    tftDisplayNoCenter(":", 2, Yellow, 240, (40*i), false,Black,0);  
                }
                readDIPorts();
                client.loop();
            }
...
void callback(char* topic, byte* payload, unsigned int length) 
{
    ...
    if (topicCallback == setRuDetail) {
        ...
        JsonObject& root = jsonBuffer.parseObject(messageBuff);
        Serial.println("I am in setWol after parsing");
     ...
     for( int i=0; i< sizeof(doPorts)/sizeof(portStruct); i++) {
        if( topicCallback == doPorts[i].portName ){
           ...
           if ( doPorts[i].curState != doPorts[i].prevState ) {
             ...
             tftDisplayNoCenterWithNumber(String(doPorts[i].curState), 2, Yellow, 250, (40*i), false,Black,0);
...
void readDIPorts()
{
     ...
     for( int i=0; i< sizeof(diPorts)/sizeof(portStruct); i++) {
        //tftDisplayNoCenterWithNumber(String(0), 2, Yellow, 100, (40*i), false,Black,0); 
