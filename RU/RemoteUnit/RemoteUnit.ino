    ...
    switch(curState) {
        ...
        case GET_RU_DETAIL:
            if(prevState == CLIENT_CONNECTED) {
                ...
                tftDisplay("Waiting For RU Detail", 2, Black ,0,80 ,true,White);
                prevState = curState;
            } else {
        ...
        case GET_PROD_ORDER_LIST:
            ...
            if(prevState == GET_AUTHENTICATION) {
                ...
                if(messageReceived == true) {
                  ...
                  Serial.println(" going out of GET_PROD_ORDER_LIST");
           }
          break;
        case GET_PROD_ORDER_DETAIL:
         Serial.println(" Entering GET_PROD_ORDER_DETAIL");
              if(prevState == GET_PROD_ORDER_LIST) {
                ...
                Serial.println(getRmlMsg);
                Serial.println(" Entering GET_PROD_ORDER_DETAIL prev state");
                prevState = curState;
              }  else {
                Serial.println("Waiting GET_PROD_ORDER_DETAIL");
                if(messageReceived == true) {
                    messageReceived = false;
        ...
        case START_PRODUCTION:
            Serial.println("Waiting START_PRODUCTION");
            myDelay(10);
        /*
            if(prevState == GET_PROD_ORDER_DETAIL) {
                Serial.println("Start Production");
                prevState = curState;
            } else {
            } */
            break;
...
void confirmStartScreen()
{
     ...
     tftDisplay("Readiness of the Machine" , 2,Yellow ,0,60, false , Black);
     tftDisplayNoCenter("ConfNo " , 2,Yellow ,40,90, false , Black);
    ...
    if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
        ...
        for(int i=0; i < sizeof(touchArea)/sizeof(touchAreaSt); i++) {
             if (( p.x >= touchArea[i].x1 && p.x <= touchArea[i].x2 && p.y >= touchArea[i].y1 && p.y <= touchArea[i].y2 )) {
                ...
                tft.drawRoundRect(touchArea[i].x1, touchArea[i].y1, touchArea[i].x2-touchArea[i].x1, touchArea[i].y2 - touchArea[i].y1, 2, Red);
                clearTouchButtons();
                return(true);
             }
        }
