...
//        myDelay(10);
            if( getRuDetailFlag == false ){
                tftDisplay("Trying to Connect",2,Black,0,60, true);
                tftDisplay("to the Gateway...",2,Black,0,90, false);
                AskRuDetail( errorJsonLen );
                if( ruIdErrorFlag == true ){
                    //display ruiderror
                    tftDisplay(errorMsgGlobal,2,Black,0,60, true);
                    //10 sec wait
                    myDelay(10);
                    tftDisplay("Trying to Connect",2,Black,0,60, true);
                    tftDisplay("to the Gateway...",2,Black,0,90, false);
...
void callback(char* topic, byte* payload, unsigned int length) 
{
    ...
    if ((topicCallback == setRuDetail) && ( !errorJsonLen ) ) {
...
void AskRuDetail(int errorJsonLen)
{
    ...
    myDelay(10);
    if ( messageReceived && topicCallback == setRuDetail && !errorJsonLen) {
        getRuDetailFlag = false;
    }
    topicCallback = "";
}
