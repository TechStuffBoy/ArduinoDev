...
boolean isFirstTimeRuWelcome = false;
  ...
  if (!client.connected()) {
    ...
    if (getRuDetailFlag == false) {
      ...
      if (!authenticationOk) {
        ...
        if ( isFirstTimeRuWelcome == true) {
          ruWelcomeScreenFlag = true;
          checkId = true;
          isFirstTimeRuWelcome = false;
        }
        if ( rfidAuthError == true) {
          boolean a = true;
          //Serial.println("I came to rfidAuthError");
          if (!isFirstTime)
          {
            tft.fillScreen(White);
            isFirstTime = true;
          ...
          if ( currentMillis3 - previousMillis3 >= interval3 ) {
            //Serial.println("I aaaaaaaaaaaaa aaaaaaaaaaaaaaaa 1");
            previousMillis3 = currentMillis3;
            tft.fillScreen(White);
            a = false;
            ruWelcomeScreenFlag = true;
            // Serial.println("I aaaaaaaaaaaaa aaaaaaaaaaaaaaaa 2");
            checkId = true;
            //Serial.println("I aaaaaaaaaaaaa aaaaaaaaaaaaaaaa 3 ");
            rfidAuthError = false;
          }
          if (a == true) {
...
void callback(char* topic, byte* payload, unsigned int length)
{
  ...
  if ( (getPayloadLength > 0)  &&  (topicCallback == rfidSubscribeStr)  && (getPayload == "REJECT") ) {
    Serial.println("<------Start1------->");
    Serial.println("I got reject from rfid database");
    Serial.println("isFirstTime = false;");
    Serial.println("isFirstTimeRuWelcome = false;");
    Serial.println("getRuDetailFlag = true;");
    Serial.println("authenticationOk = false;");
    Serial.println("rfidAuthError = true;");
  ...
  if ( (getPayloadLength > 0)  && (topicCallback == rfidSubscribeStr) && !(getPayload == "REJECT") ) {
    ...
    Serial.println(" I got name from rfid database");
    Serial.println("getRuDetailFlag = true;");
    Serial.println("authenticationOk = true;");
    Serial.println("isFirstTime = false;");
    Serial.println("startButtonPressed = false;");
...
void getId()
{
  //Serial.println("in getId fn :");
  //Serial.print("RUID String :");
  //Serial.println(ruidStr);
  byte readCard[4];
