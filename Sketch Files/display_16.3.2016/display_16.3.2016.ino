...
void callback(char* topic,byte* payload,unsigned int length)
  {
    ...
    if(welcome || companyName ||plantName || workCenterName_1 || swipe){
        screen_1(welcome,companyName,plantName,workCenterName_1,swipe);
        }
    else {
      screen_1_error(errorMsg);
      }
    getPayload="";
}  
...
void screen_1(const char* welcome,const char* companyName,const char* plantName,const char* workCenterName_1,const char* swipe)
{
  checkId = true;
...
void screen_1_error(const char* errorMsg)
{
  checkId = false;
  tft.fillScreen(White);
  tft.setTextColor(Black);
