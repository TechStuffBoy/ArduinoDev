...
void setup()
{
    ...
    while(!client.connected()) {
        ...
        client.connect(F("Arduino1","RU-<11-11-11-00-00-01>"), 1, true, F("Unexpected Disconnect"));
    }
    Serial.println (F("Success!! MQTT:") + myMacId + F(" Connected! "));
    Serial.println("Sending client state msg to Broker");
    msgInfo="{{\"Message\": 1}}";
    msgInfo.toCharArray(msgInfoBuff, 100);
    client.publish("client/state/RU_<11-11-11-00-00-01>", msgInfoBuff);
    memset(msgInfoBuff,0,100);
    Serial.println(F("Subscribe to RU_DETAIL_<11-11-11-00-00-01>"));
    client.subscribe(F("RU_DETAIL_<11-11-11-00-00-01>"));
...
void loop()
{
    if(!client.connected()) {
        Serial.println(F("Trying to connect to MQTT Broker..."));
        //parameters required for client.connect = (mqttClientId,WillTopic,willQos,willRetain,willMessage);
        client.connect("Arduino1","RU_<11-11-11-00-00-01>", 1, true, "Unexpected Disconnect");
        if(client.connected()) {
            Serial.println (F("Success!! MQTT:") + myMacId + F(" Connected! "));
            msgInfo="{{\"Message\": 1}}";
            msgInfo.toCharArray(msgInfoBuff, 100);
            client.publish("client/state/RU_<11-11-11-00-00-01>", msgInfoBuff);
            Serial.println(F("Subscribe to RU_DETAIL_<11-11-11-00-00-01>"));
            client.subscribe("RU_DETAIL_<11-11-11-00-00-01>");
        }
    } else {
...
void getID() 
{
    ...
    jsonRfid+=rfidDetail;
    jsonRfid+="\"}";
    jsonRfid.toCharArray(jsonRfidBuff,100);
    client.subscribe("<WC_CODE>/OPERATOR_AUTH");
    client.publish("RFID",jsonRfidBuff);
    Serial.println();
    Serial.println(jsonRfidBuff);
    rfidDetail="";
    jsonRfid="";
    memset(jsonRfidBuff,0,100);
