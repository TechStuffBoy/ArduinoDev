...
{      
    ...
    rotateStepper(DI_02,DI_03); //DI_02->Fwd,DI_03->Reverse
    delay(4);
}
...
void rotateStepper(byte FWD,byte REV)
{
    int forward = readDIPort(FWD);
    int reverse = readDIPort(REV);
    int timeMilli = 5;
    ...
    if( (!forward && !reverse) ){
       ...
       //Rotate Clockwise or Forward
       fwdStepper(stepsPerRevolution,timeMilli); //stepsPerRevolution,delay
    } else if( !forward && reverse ){
        //Rotate antiClockwise or reverse
        revStepper(stepsPerRevolution,timeMilli); //stepsPerRevolution,delay
    }
}
void stopStepper()
{
...
void fwdStepper(int stepsPerRevolution,int timeMilli)
{
...
void revStepper(int stepsPerRevolution,int timeMilli)
{
     writeDOPort(DO_00, true);
        writeDOPort(DO_01, false);
        writeDOPort(DO_02, false);
