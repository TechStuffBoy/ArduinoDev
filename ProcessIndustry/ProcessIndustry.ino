...
void getProductionOrderDetailsScreen(int, bool);
...
#define DI_03  34
#define DO_00  33 //pump1
#define DO_01  32 //pump2
#define DO_02  31 //stirrer
#define DO_03  30
...
boolean willRuFlag = false;
boolean noProductionOrderPending = false;
...
void loop()
{
    switch(curState) {
        case CLIENT_NOT_CONNECTED:
            ...
            client.connect(clientIdBuff, willRUBuff, 0, false, "false");
            if(client.connected()) {
        ...
        case GET_RU_DETAIL:
            if(prevState == GW_CONNECTED) {
                ...
                if( !timeOut(60,setRuDetail) ){
                   if( (messageReceived) && ( (topicCallback == errorSAPStr)  || (topicCallback == "WILL_GW")) ){
                        messageReceived = false;
                        if(topicCallback == errorSAPStr) {
                            tftDisplay(errorSAPGlobal,2,Yellow,0,60, true,Black,5000);
                            curState = GW_DISCONNECTED;
                            prevState = CLIENT_CONNECTED;
                            break;
                        } else
                        if(topicCallback == "WILL_GW"){
                            willRuFlag = true;
                            curState = GW_DISCONNECTED;
                            prevState = CLIENT_CONNECTED;
                            willRuFlag = false;
                        ...
                        Serial.println(F("I didnt recieve msg for setRuDetail, so i'm in same loop"));
                        prevState = GW_CONNECTED;
                        curState = GET_RU_DETAIL;
                    }
                    ...
                    if( topicCallback == setRuDetail ){
                        if(isJson){
                            Serial.println(F("Proper Json arrived and is parsed in callback"));
                            if( ruIdErrorFlag && isJson ){
                                ...
                                initPorts();
                                subscribe(errorSAPStr);
                                unsubscribe(setRuDetail);
                                curState = GET_AUTHENTICATION;
                             }
                        } else {
                            tftDisplay("Inalid Response",2,Orange,0,100, true,Black,3000);
        ...
        case GET_AUTHENTICATION:
            if(prevState == GET_RU_DETAIL) {
                ...
                Serial.println(("msg recd state" + String(messageReceived)));
                if(!publishRFID() && !cardSwipped)
                    break;
                if( !timeOut(60,rfidSubscribeStr)){
                    if( (messageReceived) && ((topicCallback == errorSAPStr)  || (topicCallback == "WILL_GW")) ){
                        messageReceived = false;
                        if(topicCallback == errorSAPStr) {
                            Serial.println("I came to errorSAPStr condition and printing error sap");
                            tftDisplay(errorSAPGlobal,2,Yellow,0,60, true,Black,5000);
                            Serial.println("exiting printing error sap");
                            cardSwipped = false;
                            curState = GET_AUTHENTICATION;
                        ...
                        Serial.println(F("I didnt recieve msg for rfidSubscribeStr, so i'm in same loop"));
                        prevState = GET_RU_DETAIL;
                        curState = GET_AUTHENTICATION;
                    }
                } else {
            ...
            if(prevState == GET_AUTHENTICATION) {
                ...
                if( !timeOut(60,setWolStr) ){
                    if( (messageReceived) && ((topicCallback == errorSAPStr)  || (topicCallback == "WILL_GW")) ){
                        ...
                        Serial.println(F("I didnt recieve msg for setWolStr, so i'm in same loop"));
                        prevState = GET_AUTHENTICATION;
                        curState = GET_PROD_ORDER_LIST;
                    }
                 } else {
                    ...
                    if (topicCallback == setWolStr) {
                        if( isJson) {
                            if(noProductionOrderPending) {
                                tftDisplay("No productionOrder",2,Yellow,0,80, true,Black,0);
                                tftDisplay("is Pending",2,Yellow,0,110, false,Black,0);
                                tftDisplay("For this Workcenter",2,Yellow,0,140, false,Black,5000);
                                cardSwipped = false;
                                curState = GET_AUTHENTICATION;
                                prevState = GET_RU_DETAIL;
                                noProductionOrderPending = false;
                                break;
                            } else {
                                while(1) {
                  ...
                  prevState = curState;
                    ...
                    if( !timeOut(60,setRmlStr) ){
                       if( (messageReceived) && ((topicCallback == errorSAPStr)  || (topicCallback == "WILL_GW")) ){
                        ...
                        Serial.println(F("I didnt recieve msg for setRmlStr, so i'm in same loop"));
                        prevState = GET_PROD_ORDER_LIST;
                        curState = GET_PROD_ORDER_DETAIL;
                     }
                    } else {
    ...
    if (topicCallback == setRuDetail) {
        ...
        if(isJson) {
            ...
            if(!ruidErrorJsonLen ) {
                ...
                if( companyDescGlobal == NULL ){
                    companyDescGlobal = "Company : - NA -";
                }
                workCenter_01_DescGlobal = workcenterOneDesc;
                if( workCenter_01_DescGlobal == NULL ){
                    workCenter_01_DescGlobal = "Workcenter : - NA -";
                }
                plantDescGlobal = plantDesc;
                if( plantDescGlobal == NULL ){
                    plantDescGlobal = "Plant : - NA -";
                }
                workcenterOneCodeStr = workcenterOneCode;
                if( workcenterOneCodeStr == NULL ){
                    workcenterOneCodeStr = "WcCode  : - NA -";
                }
                ruidStr = ruid;
                ruIdErrorFlag = false;
            } else {
...
void tftDisplayNoCenterWithNumber(String msg,int textSize,int color,int x, int y, boolean clearFlag,int clearScreenColor,int delayTimeout)
{
    ...
    for(unsigned int i=0 ; i < (6 - msg.length()) ;i++){
...
void clearTouchButtons()
{
    for(unsigned int i=0; i < sizeof(touchArea)/sizeof(touchAreaSt); i++) {
        touchArea[i].x1 = 0;
        touchArea[i].y1 = 0;
        touchArea[i].x2 = 0;
        touchArea[i].y2 = 0;
...
boolean buttonPressed() {
    ...
    if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
        ...
        Serial.print(F("(")); Serial.print(p.x);
        Serial.print(F(", ")); Serial.print(p.y);
        Serial.println(F(")"));
